...
function UpdateItemModal({ isOpen, onClose, item }) {
 ...
 useEffect(() => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await axios.put(`http://127.0.0.1:8000/api/items/${item.id}`, payload);
  ...
  // for showing selected item in the modal
  useEffect(() => {
    if (item) {
      setName(item.name || "");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white p-6 rounded shadow-md w-full max-w-md">
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <Button className="bg-green-700 hover:bg-green-800" onClick={handleUpdate}>Update</Button>
        </div>
      </div>
    </div>
  );
